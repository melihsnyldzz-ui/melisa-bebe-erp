...
## Stok Alan Manuel Doğrulama Checklist’i

- Bu faz Vega ekranı ile uygulamadaki read-only stok önizleme alanlarını manuel karşılaştırmak için geçici checklist ekler.
- Checklist yalnızca manuel rehberdir; gerçek stok değerlerini repoya yazmaz.
- Durum seçenekleri `Bekliyor`, `Uyumlu`, `Fark var` ve `Emin değilim` olarak ekranda değiştirilebilir.
...
## Stok Önizleme Panel Sadeleştirme

- Bu faz read-only stok önizleme ekranını daha sade panel sırasına toplar.
...
## Vega Stok Ekranı Genel Sadeleştirme

- Bu faz Vega Read-only Stok Önizleme ekranını günlük kullanıma daha uygun sade ana akışa alır.
- Ana akış güvenlik bandı, manuel önizleme butonu, son okuma özeti, arama, kolon görünürlüğü ve stok tablosu olarak düzenlenir.
- Alan etiketleri, doğrulama notları, manuel checklist ve kullanıcı test notu tek “Gelişmiş Alan Doğrulama” başlığı altında toplanır.
