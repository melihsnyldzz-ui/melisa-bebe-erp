...
function buildDashboardData({ collections, customers, payments, products, purchaseSlips, salesSlips, suppliers }) {
  ...
  const todaySales = sumByDate(salesSlips, today, "grandTotal");
  const todayCollections = sumByDate(collections, today, "amount");
  const todayPurchases = sumByDate(purchaseSlips, today, "grandTotal");
  const todayPayments = sumByDate(payments, today, "amount");
  ...
  return {
    ...
    salesChart: buildSalesChart(salesSlips),
    topProducts: buildTopProducts(salesSlips),
    tables: {
      sales: latestRows(salesSlips).map((slip) => [
        slip.slipNo,
        slip.customerName,
        formatCurrency(slip.grandTotal),
        formatTimeOrDate(slip.createdAt || slip.date),
      ]),
      purchases: latestRows(purchaseSlips).map((slip) => [
        slip.slipNo,
        slip.supplierName,
        formatCurrency(slip.grandTotal),
        formatTimeOrDate(slip.createdAt || slip.date),
      ]),
      collections: latestRows(collections).map((collection) => [
